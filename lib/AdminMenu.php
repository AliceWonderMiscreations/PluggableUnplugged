...
class AdminMenu
{
    ...
    public static function saltMenu(): void
    {
        ...
        // @codingStandardsIgnoreLine
        echo("<p>It is suggested you allow PluggableUnplugged to generate the salts for you. If you run multiple blogs and you want your users to have the same obfuscated hash between blogs, then you can manually create the salts. If you do so, make sure they are at least 18 characters long and are made up using an arrangement of many different characters.</p>\n");
    // @codingStandardsIgnoreLine
    ...
    public static function switchToArgon(): void
    {
        ...
        echo("<p>The advantage Argon2id gives over standard WordPress password hashing is only present when an attacker manages to get a dump of your password database, a fairly common occurrence with WordPress because WordPress unwisely uses a single database for everything. When the passwords hashed with Argon2id, it becomes a lot more difficult for the attacker to figure out what the actual passwords are even when they have the hashes to them.</p>");
        echo("<p>Whether you choose to enable this feature is up to you. Obviously I recommend it or it would not be here, but it does mean that removing this plugin will result in your users needing to do a password reset, therefore I could not in good conscious default to it. So the choice is yours.</p>");
